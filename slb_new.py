...
client = AcsClient("key","ass","cn-hangzhou")
mailto_list = [""]
mail_host = "smtp"
mail_user = "user"
mail_pass = "a"
...
slb_dict = {}
mail_cotent = []
exce_list = ['172.16.1.1',"172.16.2.2"]
    ...
    try:
        ...
        for ins in json.loads(response)["Instances"]["Instance"]:
            if ins["VpcAttributes"]['PrivateIpAddress']['IpAddress']:
                ecs_dict[ins["InstanceId"]] = "".join(ins["VpcAttributes"]['PrivateIpAddress']['IpAddress'])
    except:
...
def slb_list():
    try:
        ...
        for INS in json.loads(response)["LoadBalancers"]["LoadBalancer"]:
            if INS["Address"].startswith('172.16.'):
...
def one_slb_detailed(LBID):
    try:
        ...
        response = client.do_action_with_exception(request)
        BKS_ip = []
        #print json.loads(response)["BackendServers"]["BackendServer"]
        for bk in json.loads(response)["BackendServers"]["BackendServer"]:
             if bk["ServerId"] in ecs_dict:
...
def salt_custom(slb_ip,ins_ip):
    try:
        ...
        ret = salt_client.cmd(ins_ip, 'cmd.run', ['grep -w "%s" /etc/hosts' %slb_ip])
        if ret[ins_ip] != '':
