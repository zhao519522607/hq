...
<html>
        ...
        <xmp>%s</xmp>
        <a href="http://code.haoqiao.cn/diff/%s.diff" style="display:block;background-color:DodgerBlue;">代码修改详情</a>
        <style type="text/css">
...
if __name__ == '__main__':  
        ...
        if diff_dir(file_list,'branches/offline'):
            msg = write_mail(me, all_to_list, subject, content)
            send_mail(me, all_to_list, msg)
        elif diff_dir(file_list,'trunk'):
            msg = write_mail(me, all_to_list, subject, content)
            send_mail(me, all_to_list, msg)
        elif diff_dir(file_list,'trunk/online'):
            msg = write_mail(me, api_to_list, subject, content)
            send_mail(me, api_to_list, msg)
        elif diff_dir(file_list,'trunk/aa/hss'):
            msg = write_mail(me, hqs_to_list, subject, content)
            send_mail(me, hqs_to_list, msg)
        elif diff_dir(file_list,'trunk/bb:
            msg = write_mail(me, www_to_list, subject, content)
            send_mail(me, www_to_list, msg)
        else:
            pass
